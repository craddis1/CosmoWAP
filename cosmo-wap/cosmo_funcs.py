...
class CosmologicalFunctions:
    ...
    """
    def __init__(self,cosmo,survey_params={},survey_params1=None,survey_params2=None):
        """
        ...
        #if we define different tracers then get them
        if survey_params1 is None:
            self.survey1 = self.survey
        else:
            self.survey1 = SurveyFunctions(survey_params1)
        if survey_params2 is None:
            self.survey2 = self.survey
        else:
            self.survey2 = SurveyFunctions(survey_params2)
            
        #for non tracer things - bit unneat for now  
        if 'f_sky' in survey_params.keys():
            self.f_sky = survey_params['f_sky']
        else:
            self.f_sky  =  1
            
        # get overlapping redshifts - just for pk for now
        if 'z_survey' in survey_params.keys():
            if survey_params1 is None:
                self.z_survey = np.linspace(survey_params['z_survey'][0],survey_params['z_survey'][1],int(1e+5))
            else:
                if 'z_survey' in survey_params1.keys():
                    z_min = max([survey_params['z_survey'][0],survey_params1['z_survey'][0]])
                    z_max = min([survey_params['z_survey'][1],survey_params1['z_survey'][1]])
                    if z_min > z_max:
        ...
        self.Pk_NL = get_Pk_NL(k,0) #if you want HALOFIT
    ...
    def get_params(self,k1,k2,theta,zz,tracer = None,nonlinear=False):
        """
...
def numeric_ylm(func,l,m,params,derivs,betas,r=0,s=0,sigma=None,paramsPNG=[]):
    """
