...
``FullForecast`` is the main class for survey-level forecasts. Once initialised it computes Fisher matrices, SNRs and best-fit biases, and creates MCMC samplers.
...
.. py:class:: forecast.FullForecast(cosmo_funcs, kmax_func=None, s_k=2, nonlin=False, N_bins=None, bkmax_func=None, WS_cut=True, n_mu=8, n_phi=8)
   ...
   **Methods:**
   ...
   .. method:: get_cumulative_fish(param_list, per_bin_params=None, terms='NPP', cov_terms=None, pkln=None, bkln=None, t=0, r=0, s=0, all_tracer=False, verbose=True, sigma=None, bk_terms=None, bk_st=False, precondition=True, pinv_rtol=1e-10, use_cache=True, cumulative=True)
   ...
   .. method:: best_fit_bias(param, bias_term, terms='NPP', pkln=None, bkln=None, t=0, r=0, s=0, verbose=True, sigma=None)
      ...
      Systematic parameter bias from neglecting a contribution. See :doc:`bfb`.
...
Some things to know about the sampler:

- **Proposal covariance.** By default (``fisher_covmat=True``) the proposal is seeded with the
  inverse-Fisher covariance, so the chains start with the correct degenerate correlation
...
- **Per-bin nuisance parameters.** ``per_bin_params=['b_1']`` marginalises over an independent
  ...
  and correlations rather than the flat ``per_bin_bounds`` widths. The global block is the Schur
  complement either way. For multi-tracer runs the tracer-prefixed names (``Xb_1``, ``YQ``, ...)
