...
def _check_multipole(bk_func, term_cls, ell, m, cf, k1, k2, theta, zz, rtol=RTOL):
    ...
    np.testing.assert_allclose(
        np.real(numerical), np.real(analytic), atol=1e-8, rtol=rtol,
        err_msg=f"{term_cls.__name__} l={ell} m={m} real part mismatch "
                f"(k1={k1}, k2={k2}, theta={theta:.3f})")
    np.testing.assert_allclose(
        np.imag(numerical), np.imag(analytic), atol=1e-8, rtol=rtol,
        err_msg=f"{term_cls.__name__} l={ell} m={m} imag part mismatch "
                f"(k1={k1}, k2={k2}, theta={theta:.3f})")
