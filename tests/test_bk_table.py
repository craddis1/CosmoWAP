...
def _call(bk_cls, cf, args, meth="l0", **kw):
    _, k1, k2, k3, theta, zz = args
    return np.asarray(getattr(bk_cls, meth)(cf, k1, k2, k3, theta, zz, R, S, **kw), dtype=float)


def _kernel(dispatch, bk_cls, cf, args, meth="l0", **kw):
...
def _rel(got, ref):
    return np.abs(np.asarray(got, float) - np.asarray(ref, float)).max() / np.abs(ref).max()


# ── the table reproduces the kernel ───────────────────────────────────────────


@pytest.mark.parametrize("meth", ["l0", "l2"])
def test_table_matches_kernel(dispatch, bk_cls, tri, meth):
    cf = tri[0]
    ...
    assert dispatch._cache, "the table path was not taken"
    assert got.shape == ref.shape
    assert _rel(got, ref) < RTOL_MAX


def test_table_is_reused_not_rebuilt(dispatch, bk_cls, tri):
...
def test_new_cosmology_rebuilds(dispatch, bk_cls, tri, cosmo_funcs_alt):
    """Same triangles, different cosmology: each must match its own kernel."""
    cf = tri[0]
    alt = (cosmo_funcs_alt,) + tri[1:]

    ref_a = _kernel(dispatch, bk_cls, cf, tri)
    ref_b = _kernel(dispatch, bk_cls, cosmo_funcs_alt, alt)
    assert _rel(ref_a, ref_b) > 1e-3, "the two cosmologies should differ appreciably"

    dispatch.set_version(("a",))
    got_a = _call(bk_cls, cf, tri)
    dispatch.set_version(("b",))
    got_b = _call(bk_cls, cosmo_funcs_alt, alt)
    dispatch.set_version(("a",))
    again_a = _call(bk_cls, cf, tri)
...
def test_cosmology_identity_guards_a_forgotten_version_bump(dispatch, bk_cls, tri, cosmo_funcs_alt):
    ...
    _call(bk_cls, cf, tri)
    got = _call(bk_cls, cosmo_funcs_alt, alt)
...
def test_no_version_is_the_kernel_exactly(dispatch, bk_cls, tri):
    cf = tri[0]
    dispatch.set_version(None)
    a = _call(bk_cls, cf, tri)
    b = _call(bk_cls, cf, tri)
    assert np.array_equal(a, b)


def test_array_redshift_falls_back(dispatch, bk_cls, tri):
    ...
    ref = _kernel(dispatch, bk_cls, cf, args)
    dispatch.set_version(("fiducial",))
    got = _call(bk_cls, cf, args)

    assert np.array_equal(got, ref)
    assert not dispatch._cache, "the fallback must not populate the table cache"


def test_extra_kwargs_fall_back(dispatch, bk_cls, tri):
    """The table was not generated under nonlin/growth2, so those must take the kernel."""
    cf = tri[0]
    ref = _kernel(dispatch, bk_cls, cf, tri, growth2=False)
    dispatch.set_version(("fiducial",))
    got = _call(bk_cls, cf, tri, growth2=False)
...
def _fill(dispatch, bk_cls, tri, versions):
    for v in versions:
        dispatch.set_version((v,))
        _call(bk_cls, tri[0], tri)


def test_cache_holds_several_tables_within_budget(dispatch, bk_cls, tri, monkeypatch):
    """A run with several bins/methods needs more than a couple of entries resident."""
    monkeypatch.setattr(dispatch, "_CACHE_MB", 4096)
    _fill(dispatch, bk_cls, tri, ["a", "b", "c", "d", "e"])
...
def test_cache_evicts_least_recently_used_over_budget(dispatch, bk_cls, tri, monkeypatch):
    """Bounded by bytes: a budget below one table keeps exactly the newest."""
    monkeypatch.setattr(dispatch, "_CACHE_MB", 1e-3)
    _fill(dispatch, bk_cls, tri, ["a", "b", "c"])
...
def test_cache_reports_bytes_it_holds(dispatch, bk_cls, tri, monkeypatch):
    monkeypatch.setattr(dispatch, "_CACHE_MB", 4096)
    _fill(dispatch, bk_cls, tri, ["a"])
