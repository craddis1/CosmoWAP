...
def convert(mod, verbose=True):
    ...
    cls = next(CLS.match(ln).group(1) for ln in lines if CLS.match(ln))
    starts = [i for i, ln in enumerate(lines) if DEF.match(ln)]
    ...
    for j, start in enumerate(starts[:-1]):
        ...
        total = sp.expand(_inline(sp, _sympify, stmts, ret_rhs))
        t1 = time.time()
        ...
        if verbose:
            print(f'  {mod}.{meth}: {len(sp.Add.make_args(total))} terms -> {len(keys)} '
                  f'monomials, {len(body)} stmts [inline+expand {t1-t0:.0f}s, '
    ...
    dst = os.path.join(HERE, f'{mod}_tab.py')
    open(dst, 'w').write('\n'.join(out))
    if verbose:
