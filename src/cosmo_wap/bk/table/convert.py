...
def convert(mod, pkg='bk', verbose=True):
    ...
    `pkg` is the expression package - 'bk' or 'bk_mt'. Output goes to <pkg>/table/, which
    is what keeps the two apart: they share class names, so one directory would overwrite.
    """
    import sympy as sp

    sys.setrecursionlimit(100000)

    src = os.path.join(COSMO_WAP, pkg, f'{mod}.py')
    ...
    return [_convert_class(sp, mod, cls, lines, start, bounds[i + 1][0], pkg, verbose)
            for i, (start, cls) in enumerate(bounds[:-1])]


def _convert_class(sp, mod, cls, lines, cls_start, cls_end, pkg='bk', verbose=True):
    ...
    out = [f'"""Generated by cosmo_wap.bk.table.convert from {pkg}/{mod}.py - do not edit."""',
    ...
    dst_dir = os.path.join(COSMO_WAP, pkg, 'table')
    os.makedirs(dst_dir, exist_ok=True)
    dst = os.path.join(dst_dir, f'{cls}_tab.py')
    open(dst, 'w').write('\n'.join(out))
    if verbose:
