...
def bk_func(term,l,cosmo_funcs,k1,k2,k3=None,theta=None,zz=0,r=0,s=0,m=0,sigma=None,**kwargs):
    ...
    if isinstance(term, list):# so we can pass term as a list of contribtuions
        ...
        tot = []
        with cosmo_funcs.unpack_cache():# every term unpacks the same triangles - do it once
            for x in term:
...
class Full:
    ...
    @staticmethod
    def l3(cosmo_funcs,k1,k2,k3=None,theta=None,zz=0,r=0,s=0):
        return WS.l3(cosmo_funcs,k1,k2,k3,theta,zz,r,s)+bk.GR1.l3(cosmo_funcs,k1,k2,k3,theta,zz,r,s)
