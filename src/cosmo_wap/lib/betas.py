...
def interpolate_beta_funcs2(cf,tracer = None):
    """
        Function that relies on biases and functions defined above to return beta coefficients (beta expressions adapted from Eline de Weerd GitHub) from paper 1711.01812v4
    ...
    Q = tracer.Q_survey(zz)
    b_e = tracer.be_survey(zz)
    b_1 = tracer.b_1(zz)
    ...
    dH_dt2 = (1+zz)**2 *H_c**2 *cf.ddH_c(zz)+H_c*(1+zz)*(H_c+(1+zz)*cf.dH_c(zz))*cf.dH_c(zz)# can also do numerically
