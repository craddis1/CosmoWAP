...
class Sampler(BasePosterior):
    ...
    def _gaussian_prior_scales(self):
        ...
        scales = {}
        for param, spec in self.prior_dict.items():
            prior = spec.get("prior") if isinstance(spec, dict) else None
            if isinstance(prior, dict) and prior.get("dist") == "norm":
                scales[param] = prior["scale"]
        return scales
