...
class FullCovBk:
    def __init__(self,fc,cosmo_funcs_list,cov_terms,sigma=None,n_mu=64,fast=False,nonlin=False):
        ...
        cosmo_funcs,k1,k2,k3,theta,zz = fc.args
        self.kk_shape = len(kk)
        kk,zz = utils.enable_broadcasting(kk,zz,n=1) # if arrays add newaxis at the end so is broadcastable with mu!
        self.args = (cosmo_funcs,kk,zz)
    ...
    def create_cache(self,*args,**kwargs):
        ...
        | XX XY XZ |
        | YX YY YZ | 
        | ZX ZY ZZ | where YX = np.conjugate(XY) in this case -so traingle numbers compute 6 pairs!
        
        So need to store Pks for k1,k2,k3 etc"""
        ...
        for i in range(size):
            for j in range(i,size):
                 if self.cosmo_funcs_list[i][j]: # we can skip some calculation for the XY non all-tracer case
                    for term in self.terms:
                        self.pk_cache[i][j][term] = getattr(pk,term).mu(self.mu,self.cosmo_funcs_list[i][j],*args[1:],**kwargs) # so can change to new mechanism -new int
                        if i != j:
                            self.pk_cache[j][i][term] = np.conjugate(self.pk_cache[i][j][term]) # this holds currently P_YX = P_XY*
    ...
    def integrate_mu(self,i1,i2,j1,j2,k1,k2,terms,l1,l2,mu):
        ...
        For say: P_XY P_XX t1=t2=t4=0;t3=1 - P_t1t3 P_t2t4
        """
        coef = (2*l1+1)*(2*l2+1)*eval_legendre(l1,self.mu)*eval_legendre(l2,mu)*self.weights*s_B
    ...
    def get_tracer(self,a,b,c,d,e,f,terms,l1,l2):
        """Get C[B^abc_{l}, P^def_{l2}](k) - i.e. PPP term to bispectrum covariance
        C[B^abc_{l}, P^def_{l2}](k) = ((2*l1 + 1)(2*l2 + 1) s_B_123) ( Int (d(Omega_k) / 4*pi) * L_1(mu) *L_2(mu)
                                        [P^ad(k,mu)*P^be(k,mu)*P^cf(k,mu)]"""
