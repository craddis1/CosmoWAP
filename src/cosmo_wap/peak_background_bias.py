...
class PBBias:
    def __init__(self, cosmo_funcs, survey_params, HMF="Tinker2010"):
        ...
        G = GG / (1e6 * cosmo_funcs.c**2)  # gravitational constant # [Mpc M_sun^-1]
        self.rho_crit = lambda xx: (
            3 * (cosmo_funcs.H_c(xx) * (1 + xx)) ** 2 / (8 * np.pi * G)
        ...
        def get_galaxy_bias(b_h, A=1, alpha=0):
            ...
            for i, zz in enumerate(z_samps):
                bias_arr[i] = self.general_galaxy_bias(
                    b_h, zz, self.M0_func(zz), self.NO_func(zz), A, alpha
                ) / survey_params.n_g(zz)
    ...
    def general_galaxy_bias(self, b_h, zz, M0, NO, A=1, alpha=0):
        ...
        integral_values = scipy.integrate.simpson(
            b_h(zz, A, alpha) * self.n_h(zz) * self.HOD(zz, M0, NO), (self.M(zz)), axis=-1
        )
    ...
    def fit_M0(self, z_arr):  # M0 is in units of M_sun/h
        ...
        def objective(M0, zz, NO=1):
            ...
            """
            return self.general_galaxy_bias(self.eulbias.b1, zz, M0, NO) / self.number_density(
                zz, M0, NO
            ) - self.survey_params.b_1(zz)

        M0_arr = np.array([scipy.optimize.newton(objective, x0=1e12, args=(z,), rtol=1e-5) for z in z_arr])
    ...
    def fit_NO(self, z_arr):
        ...
        NO_arr = np.array(
            [scipy.optimize.newton(objective, x0=2.0, args=(z, self.M0_func(z)), rtol=1e-5) for z in z_arr]
        )
    ...
    def sigma_R_n(self, R, n, K_MIN=5e-5, steps=int(1e3)):
        ...
        def sigma_sq(R, n):
            k_limits = [K_MIN, self.cosmo_funcs.K_MAX]
            yinit = [0.0]
    ...
    class EulBias:
        ...
        def b_01(self, zz, A=1, alpha=0):
            ...
            return (
                A
                * (
                    2 * delta_c * self.pc.lagbias.b1(zz)
