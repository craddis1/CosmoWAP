...
class LxL(BaseInt):
    ...
    def l0_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l0_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=None):
            ...
            expr = 36*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)*(G*k1*(5*xd1**3 - 21*xd1**2*xd2 + 21*xd1*xd2**2 - 5*xd2**3)*np.cos(k1*(xd1 - xd2)/G) + (G**2*(-5*xd1**2 + 16*xd1*xd2 - 5*xd2**2) + 2*k1**2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**5*(xd1 - xd2)**5)
            
            return expr

        # for when xd1 = xd2
        @staticmethod
        def l0_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=None):
            zzd1, fd1, D1d1, Hd1, OMd1 = BaseInt.get_integrand_params(cosmo_functions, xd1)
            zzd2, fd2, D1d2, Hd2, OMd2 = BaseInt.get_integrand_params(cosmo_functions, xd2)
            ...
            expr = 12*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(5*G**2 + 2*k1**2*xd1*xd2)*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)/(5*G**3*d**2*k1**2)
    ...
    def l1_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l1_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=None):
            ...
            expr = 216*1j*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)*(G*(G**2*(9*xd1**2 - 33*xd1*xd2 + 9*xd2**2) - 2*k1**2*(xd1 - xd2)**2*(2*xd1**2 - 7*xd1*xd2 + 2*xd2**2))*np.sin(k1*(xd1 - xd2)/G) + k1*(xd1 - xd2)*(G**2*(-9*xd1**2 + 33*xd1*xd2 - 9*xd2**2) + k1**2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.cos(k1*(xd1 - xd2)/G))/(d**2*k1**6*(xd1 - xd2)**6)

            return expr

        # for when xd1 = xd2
        @staticmethod
        def l1_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=None):
            zzd1, fd1, D1d1, Hd1, OMd1 = BaseInt.get_integrand_params(cosmo_functions, xd1)
            zzd2, fd2, D1d2, Hd2, OMd2 = BaseInt.get_integrand_params(cosmo_functions, xd2)
            ...
            expr = -36*1j*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)*(xd1 - xd2)/(5*G**2*d**2*k1)
    ...
    def l2_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l2_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 360*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)*(G*k1*(xd1 - xd2)*(-9*G**2*(7*xd1**2 - 29*xd1*xd2 + 7*xd2**2) + k1**2*(xd1 - xd2)**2*(7*xd1**2 - 26*xd1*xd2 + 7*xd2**2))*np.cos(k1*(xd1 - xd2)/G) + (9*G**4*(7*xd1**2 - 29*xd1*xd2 + 7*xd2**2) - G**2*k1**2*(xd1 - xd2)**2*(28*xd1**2 - 113*xd1*xd2 + 28*xd2**2) + k1**4*(xd1 - xd2)**4*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**7*(xd1 - xd2)**7)
        ...
        def l2_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 24*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(7*G**2 - 2*k1**2*xd1*xd2)*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)/(7*G**3*d**2*k1**2)
    ...
    def l4_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l4_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 324*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)*(G*k1*(xd1 - xd2)*(1575*G**4*(9*xd1**2 - 46*xd1*xd2 + 9*xd2**2) - 255*G**2*k1**2*(xd1 - xd2)**2*(7*xd1**2 - 34*xd1*xd2 + 7*xd2**2) + k1**4*(xd1 - xd2)**4*(35*xd1**2 - 136*xd1*xd2 + 35*xd2**2))*np.cos(k1*(xd1 - xd2)/G) + (-1575*G**6*(9*xd1**2 - 46*xd1*xd2 + 9*xd2**2) + 30*G**4*k1**2*(xd1 - xd2)**2*(217*xd1**2 - 1094*xd1*xd2 + 217*xd2**2) - 3*G**2*k1**4*(xd1 - xd2)**4*(105*xd1**2 - 472*xd1*xd2 + 105*xd2**2) + 2*k1**6*(xd1 - xd2)**6*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**9*(xd1 - xd2)**9)
        ...
        def l4_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 72*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*xd1*xd2*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1)/(35*G**3*d**2)
...
class LxTD(BaseInt):
    ...
    def l0_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l0_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -36*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(-G*k1*(xd1 - xd2)*(d**2*(5*xd1**2 - 16*xd1*xd2 + 5*xd2**2) - 6*d*(xd1**3 - 2*xd1**2*xd2 - 2*xd1*xd2**2 + xd2**3) + 2*xd1**4 - xd1**3*xd2 - 8*xd1**2*xd2**2 - xd1*xd2**3 + 2*xd2**4)*np.cos(k1*(xd1 - xd2)/G) + (G**2*(xd1**4 + 3*xd1**3*xd2 - 14*xd1**2*xd2**2 + 3*xd1*xd2**3 + xd2**4) + d**2*(G**2*(5*xd1**2 - 16*xd1*xd2 + 5*xd2**2) - 2*k1**2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2)) - 2*d*(3*G**2 - k1**2*(xd1 - xd2)**2)*(xd1**3 - 2*xd1**2*xd2 - 2*xd1*xd2**2 + xd2**3) - 2*k1**2*xd1*xd2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**5*(xd1 - xd2)**5)
        ...
        def l0_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 12*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(15*G**4 + 5*G**2*k1**2*(d**2 - xd1**2 + xd1*xd2 - xd2**2) + 2*k1**4*xd1*xd2*(d - xd1)*(d - xd2))/(5*G**3*d**2*k1**4)
    ...
    def l1_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l1_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 108*1j*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(G*(G**2*(4*xd1**4 + 8*xd1**3*xd2 - 54*xd1**2*xd2**2 + 8*xd1*xd2**3 + 4*xd2**4) + 2*d**2*(G**2*(9*xd1**2 - 33*xd1*xd2 + 9*xd2**2) - 2*k1**2*(xd1 - xd2)**2*(2*xd1**2 - 7*xd1*xd2 + 2*xd2**2)) + 3*d*(xd1 + xd2)*(G**2*(-7*xd1**2 + 24*xd1*xd2 - 7*xd2**2) + k1**2*(xd1 - xd2)**2*(3*xd1**2 - 10*xd1*xd2 + 3*xd2**2)) - 2*k1**2*(xd1 - xd2)**2*(xd1**4 + xd1**3*xd2 - 10*xd1**2*xd2**2 + xd1*xd2**3 + xd2**4))*np.sin(k1*(xd1 - xd2)/G) - k1*(xd1 - xd2)*(G**2*(4*xd1**4 + 8*xd1**3*xd2 - 54*xd1**2*xd2**2 + 8*xd1*xd2**3 + 4*xd2**4) + 2*d**2*(G**2*(9*xd1**2 - 33*xd1*xd2 + 9*xd2**2) - k1**2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2)) + d*(xd1 + xd2)*(-3*G**2*(7*xd1**2 - 24*xd1*xd2 + 7*xd2**2) + 2*k1**2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2)) - 2*k1**2*xd1*xd2*(xd1 - xd2)**2*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.cos(k1*(xd1 - xd2)/G))/(d**2*k1**6*(xd1 - xd2)**6)
        ...
        def l1_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -36*1j*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(5*G**2 + k1**2*(d - xd1)*(d - xd2))*(Qm - 1)*(xQm - 1)*(xd1 - xd2)/(5*G**2*d**2*k1**3)
    ...
    def l2_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l2_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 180*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(-G*k1*(xd1 - xd2)*(6*G**2*(4*xd1**4 + 11*xd1**3*xd2 - 75*xd1**2*xd2**2 + 11*xd1*xd2**3 + 4*xd2**4) + 2*d**2*(9*G**2*(7*xd1**2 - 29*xd1*xd2 + 7*xd2**2) - k1**2*(xd1 - xd2)**2*(7*xd1**2 - 26*xd1*xd2 + 7*xd2**2)) + 3*d*(xd1 + xd2)*(-6*G**2*(8*xd1**2 - 31*xd1*xd2 + 8*xd2**2) + k1**2*(xd1 - xd2)**2*(5*xd1**2 - 18*xd1*xd2 + 5*xd2**2)) - 2*k1**2*(xd1 - xd2)**2*(xd1**4 + 4*xd1**3*xd2 - 22*xd1**2*xd2**2 + 4*xd1*xd2**3 + xd2**4))*np.cos(k1*(xd1 - xd2)/G) + (6*G**4*(4*xd1**4 + 11*xd1**3*xd2 - 75*xd1**2*xd2**2 + 11*xd1*xd2**3 + 4*xd2**4) - 2*G**2*k1**2*(xd1 - xd2)**2*(5*xd1**4 + 15*xd1**3*xd2 - 97*xd1**2*xd2**2 + 15*xd1*xd2**3 + 5*xd2**4) + 2*d**2*(9*G**4*(7*xd1**2 - 29*xd1*xd2 + 7*xd2**2) - G**2*k1**2*(xd1 - xd2)**2*(28*xd1**2 - 113*xd1*xd2 + 28*xd2**2) + k1**4*(xd1 - xd2)**4*(xd1**2 - 3*xd1*xd2 + xd2**2)) - d*(xd1 + xd2)*(18*G**4*(8*xd1**2 - 31*xd1*xd2 + 8*xd2**2) - 3*G**2*k1**2*(xd1 - xd2)**2*(21*xd1**2 - 80*xd1*xd2 + 21*xd2**2) + 2*k1**4*(xd1 - xd2)**4*(xd1**2 - 3*xd1*xd2 + xd2**2)) + 2*k1**4*xd1*xd2*(xd1 - xd2)**4*(xd1**2 - 3*xd1*xd2 + xd2**2))*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**7*(xd1 - xd2)**7)
        ...
        def l2_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 12*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(7*G**2*(xd1 + xd2)**2 + 2*d**2*(7*G**2 - 2*k1**2*xd1*xd2) + d*(-21*G**2 + 4*k1**2*xd1*xd2)*(xd1 + xd2) - 4*k1**2*xd1**2*xd2**2)/(7*G**3*d**2*k1**2)
...
class LxISW(BaseInt):
    ...
    def l0_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l0_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 3*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(15*G**4*Hd1*Hd2*(fd1 - 1)*(fd2 - 1)*(H**2*d*(-2*Qm + be) + 2*H*(Qm - 1) - Hp*d)*(H**2*d*(-2*xQm + xbe) + 2*H*(xQm - 1) - Hp*d) + 10*G**2*H**2*k1**2*(2*H*(Qm - 1)*(xQm - 1)*(-Hd2*xd1**2*(fd2 - 1) + xd2*(H*xd1 - Hd1*xd2*(fd1 - 1))) + d**2*(H**2*(-Hd2*xd1*(fd2 - 1)*(-2*xQm + xbe) + Qm*(Hd2*xd1*(fd2 - 1)*(-2*xQm + xbe) - 2*(xQm - 1)*(Hd1*xd2*(fd1 - 1) - 1)) + (xQm - 1)*(Hd1*be*xd2*(fd1 - 1) - 2)) + Hp*(Hd1*xd2*(-fd1*xQm + fd1 + xQm - 1) + Hd2*xd1*(-Qm*fd2 + Qm + fd2 - 1))) + d*(H**2*(-Hd2*xd1**2*(Qm - 1)*(fd2 - 1)*(-2*xQm + xbe) + 2*xd1*(-Qm*xQm + Qm + xQm - 1) + xd2*(xQm - 1)*(-Hd1*be*xd2*(fd1 - 1) + 2*Qm*(Hd1*xd2*(fd1 - 1) - 1) + 2)) + 2*H*(Qm - 1)*(xQm - 1)*(Hd1*xd2*(fd1 - 1) + Hd2*xd1*(fd2 - 1)) + Hp*(Hd1*xd2**2*(fd1 - 1)*(xQm - 1) + Hd2*xd1**2*(Qm - 1)*(fd2 - 1)))) + 8*H**4*k1**4*xd1*xd2*(Qm - 1)*(d - xd1)*(d - xd2)*(xQm - 1))/(5*G**3*H**4*d**2*k1**4)
    ...
    def l1_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l1_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -18*1j*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(2*H*(Qm - 1)*(xQm - 1)*(5*G**2*(-Hd1*xd2*(fd1 - 1) + Hd2*xd1*(fd2 - 1)) + H*k1**2*xd1*xd2*(xd1 - xd2)) + d**2*(5*G**2*H**2*Hd1*be*(fd1 - 1)*(xQm - 1) - 5*G**2*(Hd1*(fd1 - 1)*(xQm - 1)*(2*H**2*Qm + Hp) - Hd2*(Qm - 1)*(fd2 - 1)*(-H**2*(-2*xQm + xbe) + Hp)) + 2*H**2*k1**2*(Qm - 1)*(xQm - 1)*(xd1 - xd2)) + d*(5*G**2*(H**2*(-Hd1*xd2*(-2*Qm + be)*(fd1 - 1)*(xQm - 1) + Hd2*xd1*(Qm - 1)*(fd2 - 1)*(-2*xQm + xbe)) + 2*H*(Qm - 1)*(xQm - 1)*(Hd1*(fd1 - 1) - Hd2*fd2 + Hd2) + Hp*(Hd1*xd2*(fd1 - 1)*(xQm - 1) + Hd2*xd1*(-Qm*fd2 + Qm + fd2 - 1))) - 2*H**2*k1**2*(Qm - 1)*(xQm - 1)*(xd1**2 - xd2**2)))/(5*G**2*H**2*d**2*k1**3)
    ...
    def l2_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l2_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -6*D1d1*D1d2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(-2*H*(Qm - 1)*(xQm - 1)*(7*G**2*(Hd2*xd1**2*(fd2 - 1) + xd2*(2*H*xd1 + Hd1*xd2*(fd1 - 1))) - 4*H*k1**2*xd1**2*xd2**2) + d**2*(7*G**2*(H**2*(-Hd2*xd1*(fd2 - 1)*(-2*xQm + xbe) + Qm*(Hd2*xd1*(fd2 - 1)*(-2*xQm + xbe) - 2*(xQm - 1)*(Hd1*xd2*(fd1 - 1) + 2)) + (xQm - 1)*(Hd1*be*xd2*(fd1 - 1) + 4)) + Hp*(Hd1*xd2*(-fd1*xQm + fd1 + xQm - 1) + Hd2*xd1*(-Qm*fd2 + Qm + fd2 - 1))) + 8*H**2*k1**2*xd1*xd2*(Qm - 1)*(xQm - 1)) + d*(-7*G**2*(H**2*(Hd2*xd1**2*(Qm - 1)*(fd2 - 1)*(-2*xQm + xbe) + 4*xd1*(-Qm*xQm + Qm + xQm - 1) - xd2*(xQm - 1)*(-Hd1*xd2*(-2*Qm + be)*(fd1 - 1) + 4*Qm - 4)) - 2*H*(Qm - 1)*(xQm - 1)*(Hd1*xd2*(fd1 - 1) + Hd2*xd1*(fd2 - 1)) + Hp*(Hd1*xd2**2*(-fd1*xQm + fd1 + xQm - 1) + Hd2*xd1**2*(-Qm*fd2 + Qm + fd2 - 1))) - 8*H**2*k1**2*xd1*xd2*(Qm - 1)*(xQm - 1)*(xd1 + xd2)))/(7*G**3*H**2*d**2*k1**2)
...
class TDxTD(BaseInt):
    ...
    def l0_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l0_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 36*D1d1*D1d2*G*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)/(d**2*k1**4)
    ...
    def l1_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l1_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -108*1j*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(G*np.sin(k1*(xd1 - xd2)/G) - k1*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G))/(d**2*k1**6*(xd1 - xd2)**2)
    ...
    def l2_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l2_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 180*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(3*G*k1*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G) + (-3*G**2 + k1**2*(xd1 - xd2)**2)*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**7*(xd1 - xd2)**3)
    ...
    def l3_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l3_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 252*1j*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(3*G*(5*G**2 - 2*k1**2*(xd1 - xd2)**2)*np.sin(k1*(xd1 - xd2)/G) + k1*(-15*G**2 + k1**2*(xd1 - xd2)**2)*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G))/(d**2*k1**8*(xd1 - xd2)**4)
    ...
    def l4_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l4_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 324*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(Qm - 1)*(xQm - 1)*(-5*G*k1*(21*G**2 - 2*k1**2*(xd1 - xd2)**2)*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G) + (105*G**4 - 45*G**2*k1**2*(xd1 - xd2)**2 + k1**4*(xd1 - xd2)**4)*np.sin(k1*(xd1 - xd2)/G))/(d**2*k1**9*(xd1 - xd2)**5)
...
class ISWxISW(BaseInt):
    ...
    def l0_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l0_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 9*D1d1*D1d2*G*Hd1**3*Hd2**3*OMd1*OMd2*Pk*(fd1 - 1)*(fd2 - 1)*(-2*Qm + be + (2*Qm - 2)/(H*d) - Hp/H**2)*(-2*xQm + xbe + (2*xQm - 2)/(H*d) - Hp/H**2)/k1**4
    ...
    def l1_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l1_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -27*1j*D1d1*D1d2*G**2*Hd1**3*Hd2**3*OMd1*OMd2*Pk*(fd1 - 1)*(fd2 - 1)*(G*np.sin(k1*(xd1 - xd2)/G) - k1*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G))*(-2*Qm + be + (2*Qm - 2)/(H*d) - Hp/H**2)*(-2*xQm + xbe + (2*xQm - 2)/(H*d) - Hp/H**2)/(k1**6*(xd1 - xd2)**2)
    ...
    def l2_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l2_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -45*D1d1*D1d2*G**2*Hd1**3*Hd2**3*OMd1*OMd2*Pk*(fd1 - 1)*(fd2 - 1)*(3*G*k1*(-xd1 + xd2)*np.cos(k1*(xd1 - xd2)/G) + (3*G**2 - k1**2*(xd1 - xd2)**2)*np.sin(k1*(xd1 - xd2)/G))*(H**2*d*(-2*Qm + be) + 2*H*(Qm - 1) - Hp*d)*(H**2*d*(-2*xQm + xbe) + 2*H*(xQm - 1) - Hp*d)/(H**4*d**2*k1**7*(xd1 - xd2)**3)
    ...
    def l3_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l3_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 63*1j*D1d1*D1d2*G**2*Hd1**3*Hd2**3*OMd1*OMd2*Pk*(fd1 - 1)*(fd2 - 1)*(3*G*(5*G**2 - 2*k1**2*(xd1 - xd2)**2)*np.sin(k1*(xd1 - xd2)/G) + k1*(-15*G**2 + k1**2*(xd1 - xd2)**2)*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G))*(H**2*d*(-2*Qm + be) + 2*H*(Qm - 1) - Hp*d)*(H**2*d*(-2*xQm + xbe) + 2*H*(xQm - 1) - Hp*d)/(H**4*d**2*k1**8*(xd1 - xd2)**4)
    ...
    def l4_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l4_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 81*D1d1*D1d2*G**2*Hd1**3*Hd2**3*OMd1*OMd2*Pk*(fd1 - 1)*(fd2 - 1)*(-5*G*k1*(21*G**2 - 2*k1**2*(xd1 - xd2)**2)*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G) + (105*G**4 - 45*G**2*k1**2*(xd1 - xd2)**2 + k1**4*(xd1 - xd2)**4)*np.sin(k1*(xd1 - xd2)/G))*(H**2*d*(-2*Qm + be) + 2*H*(Qm - 1) - Hp*d)*(H**2*d*(-2*xQm + xbe) + 2*H*(xQm - 1) - Hp*d)/(H**4*d**2*k1**9*(xd1 - xd2)**5)
...
class TDxISW(BaseInt):
    ...
    def l0_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l0_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 9*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(H**2*(Hd1*d*(-2*Qm + be)*(fd1 - 1) + 2*Qm - 2) + 2*H*Hd1*(Qm - 1)*(fd1 - 1) - Hd1*Hp*d*(fd1 - 1))*(H**2*(Hd2*d*(fd2 - 1)*(-2*xQm + xbe) + 2*xQm - 2) + 2*H*Hd2*(fd2 - 1)*(xQm - 1) - Hd2*Hp*d*(fd2 - 1))*np.sin(k1*(xd1 - xd2)/G)/(H**4*d**2*k1**5*(xd1 - xd2))
        ...
        def l0_terms2(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = D1d1*D1d2*Pk*(3*G**2*Hd1**3*OMd1*(fd1 - 1)*(-2*Qm + be + (2*Qm - 2)/(H*d) - Hp/H**2)/k1**2 + 6*G**2*Hd1**2*OMd1*(Qm - 1)/(d*k1**2))*(3*G**2*Hd2**3*OMd2*(fd2 - 1)*(-2*xQm + xbe + (2*xQm - 2)/(H*d) - Hp/H**2)/k1**2 + 6*G**2*Hd2**2*OMd2*(xQm - 1)/(d*k1**2))/G**3
    ...
    def l1_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l1_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 27*1j*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(G*np.sin(k1*(xd1 - xd2)/G) - k1*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G))*(H**2*(-Hd1*be*d*(fd1 - 1) + 2*Qm*(Hd1*d*(fd1 - 1) - 1) + 2) - 2*H*Hd1*(Qm - 1)*(fd1 - 1) + Hd1*Hp*d*(fd1 - 1))*(H**2*(Hd2*d*(fd2 - 1)*(-2*xQm + xbe) + 2*xQm - 2) + 2*H*Hd2*(fd2 - 1)*(xQm - 1) - Hd2*Hp*d*(fd2 - 1))/(H**4*d**2*k1**6*(xd1 - xd2)**2)
    ...
    def l2_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l2_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = -45*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(3*G*k1*(-xd1 + xd2)*np.cos(k1*(xd1 - xd2)/G) + (3*G**2 - k1**2*(xd1 - xd2)**2)*np.sin(k1*(xd1 - xd2)/G))*(H**2*(Hd1*d*(-2*Qm + be)*(fd1 - 1) + 2*Qm - 2) + 2*H*Hd1*(Qm - 1)*(fd1 - 1) - Hd1*Hp*d*(fd1 - 1))*(H**2*(Hd2*d*(fd2 - 1)*(-2*xQm + xbe) + 2*xQm - 2) + 2*H*Hd2*(fd2 - 1)*(xQm - 1) - Hd2*Hp*d*(fd2 - 1))/(H**4*d**2*k1**7*(xd1 - xd2)**3)
    ...
    def l3_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l3_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 63*1j*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(3*G*(5*G**2 - 2*k1**2*(xd1 - xd2)**2)*np.sin(k1*(xd1 - xd2)/G) + k1*(-15*G**2 + k1**2*(xd1 - xd2)**2)*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G))*(H**2*(Hd1*d*(-2*Qm + be)*(fd1 - 1) + 2*Qm - 2) + 2*H*Hd1*(Qm - 1)*(fd1 - 1) - Hd1*Hp*d*(fd1 - 1))*(H**2*(Hd2*d*(fd2 - 1)*(-2*xQm + xbe) + 2*xQm - 2) + 2*H*Hd2*(fd2 - 1)*(xQm - 1) - Hd2*Hp*d*(fd2 - 1))/(H**4*d**2*k1**8*(xd1 - xd2)**4)
    ...
    def l4_integrand(xd1, xd2, cosmo_funcs, k1, zz, t=0, sigma=None, fast=True,**kwargs):
        ...
        def l4_terms1(xd1, xd2, cosmo_functions, k1, zz, t=0, sigma=0):
            ...
            expr = 81*D1d1*D1d2*G**2*Hd1**2*Hd2**2*OMd1*OMd2*Pk*(-5*G*k1*(21*G**2 - 2*k1**2*(xd1 - xd2)**2)*(xd1 - xd2)*np.cos(k1*(xd1 - xd2)/G) + (105*G**4 - 45*G**2*k1**2*(xd1 - xd2)**2 + k1**4*(xd1 - xd2)**4)*np.sin(k1*(xd1 - xd2)/G))*(H**2*(Hd1*d*(-2*Qm + be)*(fd1 - 1) + 2*Qm - 2) + 2*H*Hd1*(Qm - 1)*(fd1 - 1) - Hd1*Hp*d*(fd1 - 1))*(H**2*(Hd2*d*(fd2 - 1)*(-2*xQm + xbe) + 2*xQm - 2) + 2*H*Hd2*(fd2 - 1)*(xQm - 1) - Hd2*Hp*d*(fd2 - 1))/(H**4*d**2*k1**9*(xd1 - xd2)**5)
