...
class LxNPP(BaseInt):
    ...
    def same_mu_integrand(xd,mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128):
        ...
        expr = D1**2*Pk*(Hd**2*OMd*xd*(3*Qm - 3)*(b1 + f*mu**2)*(d - xd)*(2*1j*G*mu/(k1*xd) - mu**2 + 1)/d + Hd**2*OMd*xd*(d - xd)*(3*xQm - 3)*(f*mu**2 + xb1)*(-2*1j*G*mu/(k1*xd) - mu**2 + 1)/d)/G**3
        return expr
    
    @staticmethod
    def same_mu(mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=False):
        """2D P(k,mu) power spectra"""
        return BaseInt.single_int(LxNPP.same_mu_integrand, mu, cosmo_funcs, k1, zz, t, sigma, n=n, remove_div=remove_div)
    
    @staticmethod
    def same_l(l,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128,n_mu=16,fast=False):
        """Returns lth multipole with numeric mu integration over P(k,mu) power spectra"""
        return integrate.legendre(LxNPP.same_mu,l,cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n ,n_mu=n_mu,fast=fast)
    ...
    def l0_same_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        _,f,D1,b1,xb1 = cosmo_funcs.unpack_pk(k1,zz) # generic power spectrum params
        d, H, Hp, Qm, xQm, be, xbe = BaseInt.get_int_params(cosmo_funcs, zz) # source integrated params
        zzd1, _, _, _, _ = BaseInt.get_integrand_params(cosmo_funcs, xd) # integrand params - arrays in shape (xd)
...
class TDxNPP(BaseInt):
    ...
    def same_mu_integrand(xd,mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128):
        ...
        expr = D1**2*Pk*((b1 + f*mu**2)*(3*G**2*Hd**3*OMd*(fd - 1)*(-2*Qm + be + (2*Qm - 2)/(H*d) - Hp/H**2)/k1**2 + 3*G**2*Hd**2*OMd*(2*Qm - 2)/(d*k1**2)) + (f*mu**2 + xb1)*(3*G**2*Hd**3*OMd*(fd - 1)*(-2*xQm + xbe + (2*xQm - 2)/(H*d) - Hp/H**2)/k1**2 + 3*G**2*Hd**2*OMd*(2*xQm - 2)/(d*k1**2)))/G**3
        return expr
    
    @staticmethod
    def same_mu(mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=False):
        """2D P(k,mu) power spectra"""
        return BaseInt.single_int(TDxNPP.same_mu_integrand, mu, cosmo_funcs, k1, zz, t, sigma, n=n, remove_div=remove_div)
    
    @staticmethod
    def same_l(l,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128,n_mu=16,fast=False):
        """Returns lth multipole with numeric mu integration over P(k,mu) power spectra"""
        return integrate.legendre(TDxNPP.same_mu,l,cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n ,n_mu=n_mu,fast=fast)
...
class ISWxNPP(BaseInt):
    ...
    def same_mu(mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=False):
        """2D P(k,mu) power spectra"""
        return BaseInt.single_int(ISWxNPP.same_mu_integrand, mu, cosmo_funcs, k1, zz, t, sigma, n=n, remove_div=remove_div)
    
    @staticmethod
    def same_l(l,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128,n_mu=16,fast=False):
        """Returns lth multipole with numeric mu integration over P(k,mu) power spectra"""
        return integrate.legendre(ISWxNPP.same_mu,l,cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n ,n_mu=n_mu,fast=fast)
...
class NPPxNPP(BaseInt):
    ...
    def mu_integrand(xd,mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128):
        ...
        expr = D1*D1d*Pk*((b1 + f*mu**2)*(-1j*np.sin(k1*mu*(-d + xd)/G) + np.cos(k1*mu*(-d + xd)/G)) + (1j*np.sin(k1*mu*(-d + xd)/G) + np.cos(k1*mu*(-d + xd)/G))*(f*mu**2 + xb1))/G**3
        return expr
    
    @staticmethod
    def mu(mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=False):
        """2D P(k,mu) power spectra"""
        return BaseInt.single_int(NPPxNPP.mu_integrand, mu, cosmo_funcs, k1, zz, t, sigma, n=n, remove_div=remove_div)
    
    @staticmethod
    def l(l,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128,n_mu=16,fast=False):
        """Returns lth multipole with numeric mu integration over P(k,mu) power spectra"""
        return integrate.legendre(NPPxNPP.mu,l,cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n ,n_mu=n_mu,fast=fast)
    ...
    def same_mu_integrand(xd,mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128):
        ...
        expr = D1**2*Pk*(b1 + 2*f*mu**2 + xb1)/G**3
    ...
    def same_mu(mu,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=False):
        """2D P(k,mu) power spectra"""
        return BaseInt.single_int(NPPxNPP.same_mu_integrand, mu, cosmo_funcs, k1, zz, t, sigma, n=n, remove_div=remove_div)
    
    @staticmethod
    def same_l(l,cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128,n_mu=16,fast=False):
        """Returns lth multipole with numeric mu integration over P(k,mu) power spectra"""
        return integrate.legendre(NPPxNPP.same_mu,l,cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n ,n_mu=n_mu,fast=fast)

    ############################ Seperate Multipoles - with analytic mu integration #################################

    @staticmethod
    def l0(cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=False):
        return BaseInt.single_int(NPPxNPP.l0_integrand, cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n, remove_div=remove_div)
    ...
    def l0_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = D1*D1d*pk*(4*G*f*k1*(-d + xd)*np.cos(k1*(-d + xd)/G) + (2*f*(-2*G**2 + k1**2*(d - xd)**2) + k1**2*(b1 + xb1)*(d - xd)**2)*np.sin(k1*(-d + xd)/G))/(G**2*k1**3*(-d + xd)**3)
    ...
    def l0_same_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = D1**2*pk*(b1 + 2*f/3 + xb1)/G**3

        return expr
    
    @staticmethod
    def l1(cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=True):
        return BaseInt.single_int(NPPxNPP.l1_integrand, cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n, remove_div=remove_div)
    ...
    def l1_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = -3*1j*D1*D1d*pk*(b1 - xb1)*(G*np.sin(k1*(-d + xd)/G) + k1*(d - xd)*np.cos(k1*(-d + xd)/G))/(G**2*k1**2*(d - xd)**2)
        return expr
    ...
    def l2(cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=True):
        return BaseInt.single_int(NPPxNPP.l2_integrand, cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n, remove_div=remove_div)
    ...
    def l2_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = 5*D1*D1d*pk*(G*k1*(d - xd)*(2*f*(36*G**2 - 5*k1**2*(d - xd)**2) - 3*k1**2*(b1 + xb1)*(d - xd)**2)*np.cos(k1*(-d + xd)/G) + (2*f*(36*G**4 - 17*G**2*k1**2*(d - xd)**2 + k1**4*(d - xd)**4) + k1**2*(-3*G**2 + k1**2*(d - xd)**2)*(b1 + xb1)*(d - xd)**2)*np.sin(k1*(-d + xd)/G))/(G**2*k1**5*(-d + xd)**5)
        return expr
    ...
    def l2_same_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = 4*D1**2*pk*f/(3*G**3)
        return expr
    
    def l3(cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128, remove_div=True):
        return BaseInt.single_int(NPPxNPP.l3_integrand, cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n, remove_div=remove_div)
    ...
    def l3_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = 7*1j*D1*D1d*G*pk*(b1 - xb1)*(3*(5 - 2*k1**2*(d - xd)**2/G**2)*np.sin(k1*(-d + xd)/G) + k1*(-15 + k1**2*(d - xd)**2/G**2)*(-d + xd)*np.cos(k1*(-d + xd)/G)/G)/(k1**4*(d - xd)**4)
        return expr
    ...
    def l4(cosmo_funcs, k1, zz=0, t=0, sigma=None, n=128):
        return BaseInt.single_int(NPPxNPP.l4_integrand, cosmo_funcs, k1, zz, t=t, sigma=sigma, n=n)
    ...
    def l4_integrand(xd,cosmo_funcs, k1, zz=0, t=0, sigma=None):
        ...
        expr = -9*D1*D1d*pk*(G*k1*(d - xd)*(6*f*(1050*G**4 - 145*G**2*k1**2*(d - xd)**2 + 4*k1**4*(d - xd)**4) - 5*k1**2*(21*G**2 - 2*k1**2*(d - xd)**2)*(b1 + xb1)*(d - xd)**2)*np.cos(k1*(-d + xd)/G) + (-2*f*(-3150*G**6 + 1485*G**4*k1**2*(d - xd)**2 - 87*G**2*k1**4*(d - xd)**4 + k1**6*(d - xd)**6) - k1**2*(b1 + xb1)*(d - xd)**2*(105*G**4 - 45*G**2*k1**2*(d - xd)**2 + k1**4*(d - xd)**4))*np.sin(k1*(-d + xd)/G))/(G**2*k1**7*(-d + xd)**7)
        return expr
